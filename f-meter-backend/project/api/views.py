...
class ProductViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        if Product.objects.filter(title = request.data.get('title')).exclude(id = request.data.get("id")):
            return Response({"message":"This product already exists","status":status.HTTP_400_BAD_REQUEST},status.HTTP_400_BAD_REQUEST)
        else:
...
class UpdatePlansCategories(APIView):
    ...
    def patch(self,request,*args,**kwargs):
        ...
        if SubscriptionPlanCategory.objects.filter(title = request.data.get('title')).exclude(id=request.data.get('id')):
            return Response({"message":"This Plan Category Already Exist","status":status.HTTP_400_BAD_REQUEST},status=status.HTTP_400_BAD_REQUEST)
...
class AddPlans(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        plans = SubscriptionPlans.objects.create(
            ...
            grace_time=request.data.get("grace_time"),
    ...
    def post(self,request,*args,**kwargs):
        ...
        if subscription:
            if not  Logo.objects.filter(clients = user):
                Logo.objects.create(logo_text = request.data.get("logo_text"),
                                            clients = user,
                                            created_by = request.user
                                            )
            else:
...
class GenerateCode(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        code = SyncCode.objects.create(code =random_number)
        digits = [int(d) for d in str(random_number)]
        return Response({'random_number': digits,"status":200},status.HTTP_200_OK)
    

class ExpireCode(APIView):
    permission_classes = [permissions.AllowAny]
    parser_classes = [MultiPartParser, JSONParser]
    def get(self, request, *args, **kwargs):
        codes = SyncCode.objects.filter(is_expired=False).only("id")
        for i in codes:
            new_created_on = i.created_on + timedelta(minutes=1)
            created_on_data = new_created_on.strftime('%Y-%m-%d %H:%M:%S')
            current_datetime = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
            if created_on_data <= current_datetime:
                i.is_expired=True
            i.save()
        return Response({"status":200},status.HTTP_200_OK)

    
class MatchCode(APIView):
    permission_classes = [permissions.AllowAny]
    parser_classes = [MultiPartParser, JSONParser]
    def post(self, request, *args, **kwargs):
        if not request.data.get("code"):
            return Response({"error": "Please enter code"}, status=status.HTTP_404_NOT_FOUND)
        if not request.data.get("token"):
            return Response({"error": "Invalid user token"}, status=status.HTTP_404_NOT_FOUND)
        if not request.data.get("domain_url"):
            return Response({"error": "Invalid domain"}, status=status.HTTP_404_NOT_FOUND)
        code =SyncCode.objects.filter(code=request.data.get("code")).last()
        if code and not code.is_expired:
            code.user_token = request.data.get('token')
            code.domain_name = request.data.get('domain_url')
            code.save()
            return Response({'message': "Code verified successfully","status":200},status.HTTP_200_OK)
        else:
             return Response({"error": "Invalid code or your code has been expired"}, status=status.HTTP_404_NOT_FOUND)
    
#### 3 party api integration pending
class VerifyToken(APIView):
    permission_classes = [permissions.AllowAny]
    parser_classes = [MultiPartParser, JSONParser]
    def get(self,request,*args,**kwargs):
        code = request.query_params.get('code')
        syncing_data=SyncCode.objects.filter(code= code)
        if syncing_data:
            return Response({"status":200},status.HTTP_200_OK)
        else:
