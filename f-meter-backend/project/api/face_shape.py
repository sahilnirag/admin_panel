...
def get_user_face_shape(user_id):
    try:
        ...
        base_dir = settings.MEDIA_ROOT  # Make sure to import settings at the beginning of your file

        imagepath = os.path.join(base_dir, str(user_obj.user_image))

        # imagepath = "C:\\Users\Dell\\Documents\\Project\\Backend\\project\\media\\marauti.localhost\\User_Images\\555-210-9876_DW8h7AK.jpg"
        ...
        for i in range(1):
            if ovalsimilarity < 10 and line2 > line4:
                f_shape = "oval"
                break
            if similarity<10:
                if angle<160:
            ...
            if line4 > line2:
                if angle<160:
                    ...
                    f_shape = "oblong"
                    break
        user_obj.face_shape = f_shape  
        user_obj.save()
        return f_shape
    except Exception as e:
            f_shape = "No Face Detected"
            user_obj.face_shape = f_shape  
            user_obj.save()
            return f_shape
