...
class BlinksReportDetails(APIView):
    permission_classes = [permissions.IsAuthenticated,]
    parser_classes = [MultiPartParser,JSONParser]
    def get(self,request):
        blinks = Blink.objects.filter(user = request.user).last()
        if blinks:
            data = {
                "blink_status":blinks.blink_status,
                "full_name":request.user.full_name,
                "age": request.user.age
                }
            ...
            return Response({"error": "no data found","status":status.HTTP_404_NOT_FOUND}, status=status.HTTP_404_NOT_FOUND)

class CreateBlinksReport(APIView):
    permission_classes = [permissions.IsAuthenticated,]
    parser_classes = [MultiPartParser,JSONParser]
    def get(self,request):
        try:
        ...
        Blink.objects.create(user = user,blinks=request.data.get('blinks')).last()
